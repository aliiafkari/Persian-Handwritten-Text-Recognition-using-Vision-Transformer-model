...
def benchmark_all_eval(model, criterion, converter, opt):
    """
    Evaluation on Persian HTR validation data.
    Instead of fixed English benchmark sub-folders, this scans opt.eval_data
    directly and treats every sub-folder it finds as one evaluation set.
    If no sub-folders exist it evaluates the root path itself.
    """
    ...
    # Discover evaluation sub-folders automatically
    root = opt.eval_data
    sub_dirs = [d for d in os.listdir(root) if os.path.isdir(os.path.join(root, d))]
    if sub_dirs:
        eval_data_list = sub_dirs
        eval_paths = [os.path.join(root, d) for d in sub_dirs]
    else:
        # No sub-folders: evaluate the root path directly
        eval_data_list = [os.path.basename(root)]
        eval_paths = [root]
    ...
    for eval_name, eval_data_path in zip(eval_data_list, eval_paths):
        AlignCollate_evaluation = AlignCollate(imgH=opt.imgH, imgW=opt.imgW, keep_ratio_with_pad=opt.PAD, opt=opt)
        try:
            eval_data, eval_data_log = hierarchical_dataset(root=eval_data_path, opt=opt)
        except AssertionError:
            print(f'[Warning] No valid samples found in {eval_data_path} — skipping.')
            continue
        ...
        print(f'[{eval_name}] Acc {accuracy_by_best_model:0.3f}\t normalized_ED {norm_ED_by_best_model:0.3f}')
        log.write(f'[{eval_name}] Acc {accuracy_by_best_model:0.3f}\t normalized_ED {norm_ED_by_best_model:0.3f}\n')
        print(dashed_line)
        log.write(dashed_line + '\n')

    if total_evaluation_data_number == 0:
        print('[Error] No evaluation data was found. Check --eval_data path.')
        log.close()
        return None
...
def test(opt):
    ...
    # Cross-platform file copy (replaces Linux-only 'cp' command)
    import shutil
    try:
