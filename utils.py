...
# Persian character set used across all scripts
...
class TokenLabelConverter(object):
    ...
    def encode(self, text):
        ...
        for i, t in enumerate(text):
            # Filter out characters not in the dictionary (important for Persian fine-tuning)
            txt = [self.GO] + [c for c in list(t) if c in self.dict] + [self.SPACE]
...
def get_args(is_train=True):
    ...
    parser.add_argument('--character', type=str,
                        default=PERSIAN_CHARS, help='character label')
