    ...
    def get_batch(self):
        ...
        for i, data_loader_iter in enumerate(self.dataloader_iter_list):
            try:
                ...
                # Epoch ended — reset iterator and continue
                ...
                print(f'[get_batch] Warning: skipping batch from loader {i}: {type(e).__name__}: {e}')

        if not balanced_batch_images:
            raise RuntimeError(
                'get_batch(): all data loaders returned empty batches.\n'
                'Most likely cause: no training samples passed the character filter.\n'
                'Fix: add --data_filtering_off to your training command.'
            )
...
class LmdbDataset(Dataset):
    ...
    def __init__(self, root, opt):
        ...
        self.root = root
        self.opt = opt
        self.env = None  # Do NOT open here — LMDB env cannot be pickled
                         # on Windows multiprocessing. Open lazily per worker.

        # Open temporarily in main process just to read metadata
        ...
        with env.begin(write=False) as txn:
            ...
            if self.opt.data_filtering_off:
                ...
                for index in range(self.nSamples):
                    ...
                    # Set membership check (safer than regex for Persian/Arabic Unicode)
                    allowed = set(self.opt.character)
        ...
        env.close()  # close after reading metadata; workers reopen lazily

    def _open_lmdb(self):
        """Open LMDB lazily inside each worker process (avoids Windows pickle error)."""
        if self.env is None:
            self.env = lmdb.open(
                self.root, max_readers=32, readonly=True,
    ...
    def __getitem__(self, index):
        ...
        self._open_lmdb()  # no-op if already open in this worker
        with self.env.begin(write=False) as txn:
            ...
            # Use set membership instead of regex — regex [^...] can misinterpret
            # Persian/Arabic Unicode codepoint ranges as ranges rather than literals.
            allowed = set(self.opt.character)
...
class DataAugment(object):
    ...
    def rand_aug(self, img):
        # np.random.choice fails on lists-of-lists (inhomogeneous shape) in newer NumPy.
        # Use random.sample instead, which works on any Python list regardless of element type.
        import random as _random
        augs = _random.sample(self.augs, min(self.opt.augs_num, len(self.augs)))
        for aug in augs:
